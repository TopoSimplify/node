...
func TestIsContiguous(t *testing.T) {
	...
	g.Describe("test contig nodes", func() {
		g.It("should test contiguous", func() {
			...
			var hulls = createHullObjects([][]int{{0, 2}, {2, 6}, {6, 8}, {8, 10}, {10, 12}, {12, len(coords) - 1}}, coords)
			inters, contig, n := IsContiguous(hulls[0].Object.(*Node), hulls[1].Object.(*Node))
			...
			g.Assert(nxt.Object.(*Node).Range.AsArray()).Equal([2]int{2, 6})

			prv, nxt = Neighbours(hulls[1], nodes)
			g.Assert(prv.Object.(*Node).Range.AsArray()).Equal([2]int{0, 2})
			g.Assert(nxt.Object.(*Node).Range.AsArray()).Equal([2]int{6, 8})
		})
	})
}
